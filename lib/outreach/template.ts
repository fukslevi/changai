...
export function buildOutreachEmail(
  ...
  if (project.projectMode === "screening") {
    return buildScreeningEmail(project, allItems);
  }
...
/**
 * The screening email: one blank per quantity, no signature, no small talk.
 *
 * Deliberately not the full RFQ email above - there is no specification depth
 * or differentiation to defend yet, only the question of whether a price near
 * the target exists at all. The fill-in-the-blank shape is the point: it asks
 * for a number in the same breath it would take to explain what one is, and a
 * factory can answer it from the subject line down without composing a reply.
 */
function buildScreeningEmail(
  ...
  const priced = allItems.filter((i) => i.kind === "priced_variant");
  const main = priced[0];
  const productLine = main?.name ?? project.name;
  const tiers = project.quantityTiers;

  const subject = `${project.name} - please quote your price`;

  const lines: string[] = [
    "Hi,",
    "",
    `Please quote your best price for this product: ${productLine}.`,
    "",
    ...tiers.map((qty) => `${qty.toLocaleString("en-US")} pcs: $___ EXW`),
    "Include Custom logo/packaging, a5, instructions, color box",
    "",
    "Also send:",
    "MOQ: ___",
    "Lead time: ___ days",
    "",
    "Please send your best price directly so we can move on quickly",
  ];
